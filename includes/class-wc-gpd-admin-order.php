...
class WC_GPD_Admin_Order {
	...
	public function render_meta_box( $post_or_order ) {
		...
		foreach ( $design_items as $item_id => $data ) {
			...
			$item         = $data['item'];
			$svg          = $data['svg'];
			$preview_svg  = WC_GPD_SVG_Sanitizer::sanitize( $svg );
			$product_name = $item->get_name();
			$download_url = $this->get_download_url( $order->get_id(), $item_id );

			?>
			<div class="wc-gpd-order-design">
				<h4><?php echo esc_html( $product_name ); ?></h4>
				<?php if ( $preview_svg ) : ?>
					<div class="wc-gpd-order-design__preview" aria-hidden="true">
						<?php
						// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- sanitized SVG preview for shop managers.
						echo $preview_svg;
						?>
					</div>
				<?php endif; ?>
				<p>
					<a href="<?php echo esc_url( $download_url ); ?>" class="button button-primary button-large wc-gpd-download-svg">
	 ...
	 * @return string
	 */
	private function get_download_url( $order_id, $item_id ) {
		return wp_nonce_url(
			add_query_arg(
				array(
